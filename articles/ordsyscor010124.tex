...
\newcommand{\path}{/home/setzer/}
\usepackage{amsfonts}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{\path/texinput/dina4}
\usepackage{pictex}
...
A {\it class} is an object $\{ x \mid \varphi \}$, where $\varphi$ is a 
...
$\{ x \mid Q(x) \}$.\\
%%%% New
A {\it binary relation} $\prec$ is a class. For a binary relations $\prec$ we 
define $r \prec s:= \pi(r,s) \in \prec$, where $\pi$ is a standard primitive
recursive pairing function on the natural numbers having the usual 
properties.\\
%%%% \end New
...
%(like $\varphi_xy$, $\psi(x)$, $(x,y,z)$, $x+y$, $\binom{x}{y}$)
%and will write in this case the result of replacing in $t$
%the variable $x_i$ by $A_i$ instead of $f[A_1 ,\ldots, A_m]$
...
%In case of $({\binom{B}{A}})^\ast_\des$,
%we write $\left(\begin{array}{ccc}b_1& \cdots& b_m \\
%a_1  &\cdots &a_m \end{array} \right)$ 
%instead of the element$\left ({\binom{b_1}{a_1}} \cdots 
%{\binom{b_m}{a_m}}\right )$ of this ordering.
%\ssubitem e
%If $A_1 ,\ldots, A_m$ are orderings and $|A_i|$ are  disjoint, then\\
%$B:= $\underlinedef{$A_1 \preccirc A_2 \preccirc \cdots \preccirc A_m$}
%is the ordering with
...
%if $m>1$ or $m=1 \land A_m \not = 0,{\binom{\hbox{\quad}}{ \hbox{\quad}}}$,
%$\iota_i{\binom{\hbox{\quad}}{\hbox{\quad}}}:= 0$,
%$\iota_1 {\binom{\alpha}{0}}:= \alpha$,
%$\iota_i+2{\binom{\alpha}{{\hbox{\quad}\hbox{\quad}}}}
%:= \alpha$. 
...
$a_1 ,\ldots, a_m \not = \underline{0}$,
$t_1:= \phibar{\binom{a_1 \cdots a_m}{ b_1 \cdots b_m}}\in \T''$,
...
$t_1 \in \T'$,\\ 
$f(t_1):= \phibar{\binom{\eval(f(a_1)) \cdots \eval(f(a_m))}{
...
%$(\Mbb_0,<_0):= \Ord$, 
%$(\Mbb_{k+1},<_{k+1}):= 
%{\binom{(\Ord \setminus \{ 0 \},<) }{(\Mbb_m,<_m)}^\ast_\des} $\\
...
%%% Local Variables: 
%%% mode: latex
%%% tex-dvi-view-command: "xdvi -bg khaki -expert -copy -thorough -s 5" 
%%% TeX-dvi-file: "ordsyscor010124b"
%%% End: 
